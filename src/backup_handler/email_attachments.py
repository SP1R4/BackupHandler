...
def _load_email_config():
    ...
    # "YOUR_APP_PASSWORD" is the template placeholder, not a credential.
    if not app_password or app_password == "YOUR_APP_PASSWORD":  # noqa: S105 # nosec B105
        ...
        try:
            ...
            if logger:
                ...
                if file_type == "application/octet-stream":
                    logger.warning(
                        f"Unknown file type for {attachment_path}. Using default MIME type 'application/octet-stream'."
                    )
    ...
    for attempt in range(1, _SMTP_MAX_RETRIES + 1):
        try:
            ...
            if attempt < _SMTP_MAX_RETRIES:
                if logger:
                    logger.warning(
                        f"SMTP error on attempt {attempt}/{_SMTP_MAX_RETRIES}: {e}. Retrying in {_SMTP_RETRY_DELAY}s..."
                    )
